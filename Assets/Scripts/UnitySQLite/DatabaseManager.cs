...
namespace UnitySQLite
{
    public class DatabaseManager : GenericSingleton<DatabaseManager>, IDisposable
    {
        ...
        /// </summary>
        public void ThreadedWriteToDatabase(string TableName, TableRow row, bool check = false, Action onEntyExists = null, Action onSuccessfulWrite = null)
        {
            ...
            if (check)
            {
                bool exists = CheckIfEntryExists(TableName, row.GetValues());
                if (exists)
                {
            ...
            WriteToDatabase(TableName, row, onSuccessfulWrite);
            //write to log file
